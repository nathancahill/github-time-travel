...
function pushStatePatch (state) {
    // Github uses pushState for navigation within a repo (actually pjax, but
    // pushState is the underlying API). We have to add an onpushstate listener
    // to add our functionality to the DOM when the page changes.
    // This requires monkey-patching the window.history.pushState function.
...
history.onpushstate = function (state) {
    // When navigation happens, we have to wait for the DOM to change before
    // attaching the date button.

    setDomChangeTimeout(attachDateButton, 'commit-group-title', 2000);
}
...
attachDateButton();
exportFunction(pushStatePatch, unsafeWindow.history, {defineAs: 'pushState', allowCallbacks: true});
